...
void* MsgDispatcher::Listen(void* arg) {
	...
	//TODO: Make accept non-blocking
        ...
        LOGIT("Start Run thread\n");
        CHECK_PTHREAD_CALL(pthread_create, (&dispatcher->t_clients_[dispatcher->nclients_], nullptr, &Run, channel), "Failed to create client handler thread");
    }
...
void* MsgDispatcher::Run(void* arg) {
    ...
    if (channel->id_ > 0) {
        //Detach and mark the end of this thread
        CHECK_PTHREAD_CALL(pthread_detach, (pthread_self()), "Failed to detach the client thread");
